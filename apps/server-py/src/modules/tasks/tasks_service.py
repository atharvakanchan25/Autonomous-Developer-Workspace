...
from src.realtime.events import TaskUpdatedPayload
...
async def create_task(body: CreateTaskRequest, user: AuthUser = Depends(get_current_user)):
    ...
    if assigned_to and not user.is_admin() and assigned_to != user.uid:
...
async def update_task_status(task_id: str, body: UpdateTaskStatusRequest, user: AuthUser = Depends(get_current_user)):
    """Update task status."""
    doc = db.collection("tasks").document(task_id).get()
    if not doc.exists:
        raise not_found("Task")
...
async def assign_task(task_id: str, body: AssignTaskRequest, user: AuthUser = Depends(get_current_user)):
