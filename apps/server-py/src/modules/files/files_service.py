...
- **Last Updated**: {project.get('updatedAt', 'Unknown')}

## Generated by Autonomous Developer Workspace

This project was automatically generated by ADW.
All code, tests, and documentation were created by AI agents.
"""
        ...
        for doc in files_snap:
            ...
            file_path = file_data.get("path", file_data.get("name", "unknown.txt"))
            content = file_data.get("content", "")
            
            # Ensure proper path structure
            # If path doesn't contain directory separators, organize by file type
            if "/" not in file_path and "\\" not in file_path:
                # Organize files by type if they're in root
                name = file_data.get("name", "file.txt")
                if name.startswith("test_"):
                    file_path = f"tests/{name}"
                elif name.endswith("_review.md"):
                    file_path = f"reviews/{name}"
                else:
            ...
            # Add to zip with project name as root directory
