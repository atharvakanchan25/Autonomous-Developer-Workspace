...
export const api = {
  ...
  files: {
    list: (projectId: string) => request<ProjectFile[]>(`/api/files/?projectId=${projectId}`),
    ...
    listVersions: (id: string) => request<FileVersionMeta[]>(`/api/files/${id}/versions`),
    getVersion: (_id: string, versionId: string) => request<FileVersion>(`/api/files/versions/${versionId}`),
  ...
  cicd: {
    ...
    list: (projectId: string) => request<Deployment[]>(`/api/cicd/deployments?projectId=${projectId}`),
  ...
  admin: {
    projects: () => request<Array<Project & { ownerId: string; ownerEmail: string; taskCount: number }>>("/api/admin/projects"),
  },
};
