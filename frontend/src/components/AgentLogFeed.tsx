...
const AGENT_COLORS: Record<string, string> = {
  CODE_GENERATOR: "text-white font-medium",
  TEST_GENERATOR: "text-gray-300",
  CODE_REVIEWER:  "text-gray-400",
};
...
export function AgentLogFeed({ projectId }: { projectId: string | null }) {
  ...
  return (
    <motion.div
      className="flex h-full flex-col border-l border-[var(--border)] bg-[var(--bg-elevated)]"
      initial={{ opacity: 0, x: 12 }}
      animate={{ opacity: 1, x: 0 }}
      transition={{ duration: duration.slow, ease: ease.enter }}
    >
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-2.5 bg-[var(--surface-soft)] backdrop-blur-sm">
        <div className="flex items-center gap-2">
          ...
          <AnimatePresence>
            {logs.length > 0 && (
              <motion.span
                className="rounded-full bg-[var(--accent-deep)]/40 px-1.5 py-0.5 text-[10px] font-medium text-[var(--accent)]"
        ...
        <div className="flex items-center gap-1">
          <motion.button
            ...
            className="rounded-lg px-2 py-1 text-[10px] text-[var(--text-muted)] transition-colors hover:bg-[var(--accent)]/10 hover:text-[var(--accent)]"
            whileTap={{ scale: 0.95 }}
          >
            {paused ? "▶" : "⏸"}
          </motion.button>
          <motion.button
            onClick={() => setLogs([])}
            className="rounded-lg px-2 py-1 text-[10px] text-[var(--text-muted)] transition-colors hover:bg-[var(--danger)]/10 hover:text-[var(--danger)]"
            whileTap={{ scale: 0.95 }}
          >
            Clear
          </motion.button>
      ...
      {/* Log entries */}
      <div className="scrollbar-thin flex-1 overflow-y-auto px-4 py-3 font-mono text-[11px] leading-relaxed bg-[var(--bg-elevated)]">
        {logs.length === 0 ? (
          <motion.p
          ...
          <AnimatedList>
            {logs.map((entry) => (
              <div key={entry.id} className="flex gap-2.5 py-0.5">
                <span className="shrink-0 text-[var(--text-muted)]">
                  {new Date(entry.timestamp).toLocaleTimeString()}
                </span>
                <span className={`w-20 shrink-0 truncate ${AGENT_COLORS[entry.agentType] ?? "text-[var(--text-muted)]"}`}>
                  {entry.agentType}
                </span>
